package com.zyqhi.applock.database;
...
public class ProtectedApplicationsTable {
	...
	public long createRow(String applicationName, String applicationPackage) {
		...
		if(rows.size() <= 0) {
			...
			//Toast for debug
			Toast.makeText(mContext, "create row succeed", Toast.LENGTH_SHORT).show();
		} else {
			insertId = 0;
			//Toast for debug
			Toast.makeText(mContext, "create row failed", Toast.LENGTH_SHORT).show();
