...
import com.zyqhi.applock.lockpattern.ConfirmLockPattern;
...
import com.zyqhi.applock.lockpattern.LockPatternView.DisplayMode;

/**
 * If the user has a lock pattern set already, makes them confirm the existing
 * one.
 * 
 * Then, prompts the user to choose a lock pattern: - prompts for initial
...
public class ChooseLockPattern extends Activity implements View.OnClickListener {
	 ...
	 */
	enum LeftButtonMode {
		Cancel(R.string.lock_cancel, true), CancelDisabled(
				R.string.lock_cancel, false), Retry(
	...
	protected void confirmPattern() {
		Toast.makeText(getApplicationContext(), "pattern cofirmed", Toast.LENGTH_SHORT).show();
		final Intent intent = new Intent(this, ConfirmLockPattern.class);
		// intent.setClassName("com.phillyandroid.applock.lockpattern",
		// "com.phillyandroid.applock.lockpattern.ConfirmLockPattern");
		startActivityForResult(intent, 55);
		//用户输入正确的pattern以后，ConfirmLockPattern会正确返回，此时这个Activity也可以结束了，返回用户程序
		finish();
		//Toast.makeText(getApplicationContext(), "startActivityForResult" + 55, Toast.LENGTH_SHORT).show();
	}
