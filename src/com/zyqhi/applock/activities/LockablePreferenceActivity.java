...
public class LockablePreferenceActivity extends PreferenceActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mLockPatternUtils.setVisiblePatternEnabled(mPrefs.getBoolean(
				"patternlock_visible_pattern", true));
		mLockPatternUtils.setTactileFeedbackEnabled(mPrefs.getBoolean(
				"patternlock_tactile_feedback", false));
	}
	...
	protected void onPause() {
		...
		if (!mLockPatternUtils.isLockPatternEnabled())
			return;
		/*
		 * Save the current time If a lock pattern has been set If this activity
		 * never loaded set the lock time to 10 seconds ago. This is to prevent
		 * the following scenario: 1. Activity Starts 2. Lock screen is
		 * displayed 3. User presses the home button 4. "lock time" is set in
		 * onPause to when the home button was pressed 5. Activity is started
		 * again within 2 seconds and no lock screen is shown this time.
		 */
		if (mHasLoaded) {
			writeLockTime();
		} else {
	...
	private void launchPatternLock() {
		...
		intent.putExtra(ConfirmLockPattern.HEADER_TEXT,
				getText(R.string.patternlock_header));
